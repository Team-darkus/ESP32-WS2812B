...
#define WIFI_RETRY_INTERVAL 10000  // 10 seconds between reconnect attempts
#define AP_TIMEOUT 20000            // 20 seconds without WiFi -> start AP mode
#define SAVE_DELAY 5000              // delay before saving to EEPROM (ms)
...
// Control modes:
// 0 - all separate
// 1 - 2+3 together (zone 2 controls both 2 and 3, zone 3 hidden)
// 2 - all together except monitor (zone 1 controls 1,2,3; zone 4 separate)
int controlMode = 1;
...
void applyEffect(CRGB* leds, int n, String mode, CRGB col,
  ...
  if (mode == "off") {
    ...
    for (int i = 0; i < n; i++) {
      ...
      uint8_t hueFire = map(noise, 0, 255, 0, 30);
...
<body>
    ...
    <div id="panel2" class="panel">
        <div class="section-title">ZONE 2 EFFECTS</div>
        <div class="mode-grid" id="modeGrid2"></div>
        <script>
            document.getElementById('modeGrid2').innerHTML = 
            ['off','solid','rainbow','wave','fade','strobe','sparkle','fire','water','bounce','pulse',
             'running','meteor','chase','rainbowMeteor','sparkles','rainbowStripes','lightning',
             'sinelon','bpm','juggle','glitter','ripple','twinkle','theaterChase','confetti','noise','matrix','colorWaves']
            .map(m => `<button class="effect-btn ${m==='off'?'off':''}" onclick="setMode(2,'${m}')">${m.toUpperCase()}</button>`).join('');
        </script>
        <div class="color-section">
            <div class="color-label"><span>COLOR</span><span class="color-value" id="colorValue2">#FFFFFF</span></div>
            <input type="color" class="color-picker" id="colorPicker2" value="#ffffff" onchange="setColor(2, this.value)">
        </div>
        <div class="slider-section">
            <div class="slider-header"><span>BRIGHTNESS</span><span class="slider-value" id="brightnessValue2">50%</span></div>
            <input type="range" min="0" max="255" value="128" class="slider" id="brightnessSlider2" oninput="updateBrightness(2, this.value)" onchange="setBrightness(2, this.value)">
        </div>
        <div class="slider-section">
            <div class="slider-header"><span>SPEED</span><span class="slider-value" id="speedValue2">128</span></div>
            <input type="range" min="0" max="255" value="128" class="slider" id="speedSlider2" oninput="updateSpeed(2, this.value)" onchange="setSpeed(2, this.value)">
        </div>
    </div>

    <!-- ZONE 3 PANEL -->
    <div id="panel3" class="panel">
        <div class="section-title">ZONE 3 EFFECTS</div>
        <div class="mode-grid" id="modeGrid3"></div>
        <script>
            document.getElementById('modeGrid3').innerHTML = 
            ['off','solid','rainbow','wave','fade','strobe','sparkle','fire','water','bounce','pulse',
             'running','meteor','chase','rainbowMeteor','sparkles','rainbowStripes','lightning',
             'sinelon','bpm','juggle','glitter','ripple','twinkle','theaterChase','confetti','noise','matrix','colorWaves']
            .map(m => `<button class="effect-btn ${m==='off'?'off':''}" onclick="setMode(3,'${m}')">${m.toUpperCase()}</button>`).join('');
        </script>
        <div class="color-section">
            <div class="color-label"><span>COLOR</span><span class="color-value" id="colorValue3">#FFFFFF</span></div>
            <input type="color" class="color-picker" id="colorPicker3" value="#ffffff" onchange="setColor(3, this.value)">
        </div>
        <div class="slider-section">
            <div class="slider-header"><span>BRIGHTNESS</span><span class="slider-value" id="brightnessValue3">50%</span></div>
            <input type="range" min="0" max="255" value="128" class="slider" id="brightnessSlider3" oninput="updateBrightness(3, this.value)" onchange="setBrightness(3, this.value)">
        </div>
        <div class="slider-section">
            <div class="slider-header"><span>SPEED</span><span class="slider-value" id="speedValue3">128</span></div>
            <input type="range" min="0" max="255" value="128" class="slider" id="speedSlider3" oninput="updateSpeed(3, this.value)" onchange="setSpeed(3, this.value)">
        </div>
    </div>

    <!-- MONITOR PANEL -->
    <div id="panel4" class="panel">
        <div class="section-title">MONITOR (GPIO19)</div>
        <div class="mode-grid" id="modeGrid4"></div>
        <script>
            document.getElementById('modeGrid4').innerHTML = 
            ['off','solid','rainbow','wave','fade','strobe','sparkle','fire','water','bounce','pulse',
             'running','meteor','chase','rainbowMeteor','sparkles','rainbowStripes','lightning',
             'sinelon','bpm','juggle','glitter','ripple','twinkle','theaterChase','confetti','noise','matrix','colorWaves']
            .map(m => `<button class="effect-btn ${m==='off'?'off':''}" onclick="setMode(4,'${m}')">${m.toUpperCase()}</button>`).join('');
        </script>
        <div class="color-section">
            <div class="color-label"><span>COLOR</span><span class="color-value" id="colorValue4">#FFFFFF</span></div>
            <input type="color" class="color-picker" id="colorPicker4" value="#ffffff" onchange="setColor(4, this.value)">
        </div>
        <div class="slider-section">
            <div class="slider-header"><span>BRIGHTNESS</span><span class="slider-value" id="brightnessValue4">50%</span></div>
            <input type="range" min="0" max="255" value="128" class="slider" id="brightnessSlider4" oninput="updateBrightness(4, this.value)" onchange="setBrightness(4, this.value)">
        </div>
        <div class="slider-section">
            <div class="slider-header"><span>SPEED</span><span class="slider-value" id="speedValue4">128</span></div>
            <input type="range" min="0" max="255" value="128" class="slider" id="speedSlider4" oninput="updateSpeed(4, this.value)" onchange="setSpeed(4, this.value)">
        </div>
    </div>
    ...
    <script>
        ...
        function switchZone(z) {
            ...
            document.querySelector(`.zone-tab[data-zone="${z}"]`).classList.add('active');
            document.querySelectorAll('.panel').forEach(p => p.classList.remove('active'));
            document.getElementById('panel' + z).classList.add('active');
            document.getElementById('zoneStatus').innerText = z;
        }
        ...
        function setControlMode(m) {
            ...
            if (m === 0) {
                zoneTabs.forEach(t => t.style.display = 'block');
                if (currentZone === 2 || currentZone === 3 || currentZone === 4) switchZone(currentZone);
                else switchZone(1);
            } else if (m === 1) {
                zoneTabs[0].style.display = 'block';
                zoneTabs[1].style.display = 'block';
                zoneTabs[2].style.display = 'none';
                zoneTabs[3].style.display = 'block';
                if (currentZone === 3) switchZone(2);
                else if (currentZone === 2 || currentZone === 1 || currentZone === 4) switchZone(currentZone);
                else switchZone(1);
            } else if (m === 2) {
                zoneTabs[0].style.display = 'block';
                zoneTabs[1].style.display = 'none';
                zoneTabs[2].style.display = 'none';
                zoneTabs[3].style.display = 'block';
                if (currentZone === 2 || currentZone === 3) switchZone(1);
                else if (currentZone === 1 || currentZone === 4) switchZone(currentZone);
                else switchZone(1);
            }
            
            fetch('/controlMode?value=' + m);
        }

        function setMode(z, m) {
            fetch('/mode?zone=' + z + '&value=' + m);
            document.getElementById('modeStatus').innerText = m.toUpperCase();
            document.querySelectorAll('#panel' + z + ' .effect-btn').forEach(btn => btn.classList.remove('active'));
            if (event) event.target.classList.add('active');
        }

        function setColor(z, c) {
            fetch('/color?zone=' + z + '&value=' + c.substring(1));
            document.getElementById('colorValue' + z).innerText = c.toUpperCase();
        }

        function updateBrightness(z, v) {
            document.getElementById('brightnessValue' + z).innerText = Math.round(v/255*100) + '%';
        }
        function setBrightness(z, v) {
            fetch('/brightness?zone=' + z + '&value=' + v);
        }

        function updateSpeed(z, v) {
            document.getElementById('speedValue' + z).innerText = v;
        }
        function setSpeed(z, v) {
            fetch('/speed?zone=' + z + '&value=' + v);
        }

        function toggleSchedule() {
            let enabled = document.getElementById('scheduleToggle').checked;
            updateSchedule();
        }
        ...
        function updateSchedule() {
            ...
            if (onTime.length==2 && offTime.length==2) {
                fetch('/schedule?enabled=' + (enabled?'1':'0') +
                      '&onHour=' + parseInt(onTime[0]) + '&onMin=' + parseInt(onTime[1]) +
                      '&offHour=' + parseInt(offTime[0]) + '&offMin=' + parseInt(offTime[1]));
            }
        }
        ...
        function checkNightMode() {
            ...
            let isNight = (hours >= 23 || hours < 6);
            document.getElementById('nightOverrideSection').style.display = isNight ? 'flex' : 'none';
        }

        function overrideNightMode() {
            fetch('/nightOverride?value=1');
            document.getElementById('nightOverrideSection').style.display = 'none';
            alert('Night mode override enabled. Brightness will be at your set level.');
        }
        ...
        function openSettings() {
            ...
            fetch('/getLEDCounts').then(r=>r.json()).then(d=>{
                document.getElementById('leds1').value = d.leds1;
                document.getElementById('leds2').value = d.leds2;
                document.getElementById('leds3').value = d.leds3;
                document.getElementById('leds4').value = d.leds4;
            });
        }
        ...
        function saveSettings() {
            let l1 = document.getElementById('leds1').value;
            let l2 = document.getElementById('leds2').value;
            let l3 = document.getElementById('leds3').value;
            let l4 = document.getElementById('leds4').value;
            fetch('/setLEDCounts?l1='+l1+'&l2='+l2+'&l3='+l3+'&l4='+l4);
        ...
        function updateCurrentTime() {
            let now = new Date();
            document.getElementById('currentTime').innerText = now.toLocaleTimeString('ru-RU', {hour12: false});
        }
        setInterval(updateCurrentTime, 1000);

        window.onload = function() {
            fetch('/status').then(r=>r.json()).then(d=>{
                document.getElementById('ipDisplay').innerText = d.ip;
                for(let i=1;i<=4;i++) {
                    let bs = document.getElementById('brightnessSlider'+i);
                    if(bs) { bs.value = d['brightness'+i]; updateBrightness(i, d['brightness'+i]); }
                    let ss = document.getElementById('speedSlider'+i);
                    if(ss) { ss.value = d['speed'+i]; updateSpeed(i, d['speed'+i]); }
                }
                document.getElementById('colorPicker1').value = '#'+d.color1;
                document.getElementById('colorPicker2').value = '#'+d.color2;
                document.getElementById('colorPicker3').value = '#'+d.color3;
                document.getElementById('colorPicker4').value = '#'+d.color4;
                document.getElementById('colorValue1').innerText = '#'+d.color1;
                document.getElementById('colorValue2').innerText = '#'+d.color2;
                document.getElementById('colorValue3').innerText = '#'+d.color3;
                document.getElementById('colorValue4').innerText = '#'+d.color4;
                
                if (d.scheduleEnabled !== undefined) {
                    document.getElementById('scheduleToggle').checked = d.scheduleEnabled;
                }
                if (d.onTime) {
...
void setup() {
  ...
  // Startup test sequence (always at 10% brightness to avoid blinding)
  int testBright = 25;
